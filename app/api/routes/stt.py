...
async def get_speech_provider(
    ...
    try:
        if provider == "openai":
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported STT provider: {provider}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to initialize STT provider: {str(e)}"
        )
...
async def get_available_providers(
    ...
    if not available_providers:
        raise HTTPException(
            status_code=500,
            detail="No STT providers configured"
        )
...
async def transcribe_audio_auto(
    ...
    if not available_providers:
        raise HTTPException(
            status_code=500,
            detail="No STT providers available"
        )
    
    # Choose provider
    provider = preferred_provider if preferred_provider in available_providers else available_providers[0]
...
async def transcribe_base64_audio(
    ...
    try:
        ...
        try:
            ...
            # Extract word-level timestamps if available
            words_data = None
            if (request.timestamps or request.word_timestamps) and hasattr(response, 'words') and response.words:
                words_data = []
                for word in response.words:
                    # Handle both dict and object formats
                    if isinstance(word, dict):
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Base64 transcription failed: {str(e)}"
        )
...
async def transcribe_url_audio(
    ...
    try:
        ...
        if not all([parsed_url.scheme, parsed_url.netloc]):
            raise HTTPException(
                status_code=400,
                detail="Invalid audio URL provided"
            )
